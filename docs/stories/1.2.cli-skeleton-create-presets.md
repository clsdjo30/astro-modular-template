...
## Tasks / Subtasks
- [ ] Implement CLI entry and `create` command (AC: 1, 2, 4)
  - [ ] Add `packages/cli` command handler and argument parsing.
  - [ ] Prompt for project name/path when not provided (plain prompts only).
  - [ ] Print a summary of the generated project path and next steps.
- [ ] Wire CLI to base template (AC: 1, 3)
  - [ ] Copy `templates/base/` into a new project directory.
  - [ ] Ensure output is a buildable Astro + TypeScript project.
  - [ ] Do not include module system or preset-specific content in this story.
- [ ] Add CLI tests (AC: 1, 2, 4)
  - [ ] Unit tests for argument parsing and path handling.
  - [ ] Integration test: `create` produces expected base template file tree.
- [ ] Update CLI docs (AC: 4)
  - [ ] Add usage to README or CLI help output.
