...
## Tasks / Subtasks
- [x] Scaffold API service package (AC: 1, 3)
  - [x] Create `packages/api/` with `src/app.ts` and `src/server.ts`.
  - [x] Add basic Express app with JSON middleware and health routes.
  - [x] Add `GET /healthz` and `GET /readyz` endpoints.
  - [x] Ensure API package is referenced only by backend-enabled presets.
- [x] Add database layer (AC: 2)
  - [x] Add Knex configuration in `packages/api/src/db/index.ts`.
  - [x] Support MariaDB by default with PostgreSQL as an option.
  - [x] Add a minimal migrations folder scaffold.
- [x] Add core middleware (AC: 1)
  - [x] Add error middleware that normalizes responses to `{ error: { code, message, details? } }`.
  - [x] Add request ID logging via Pino.
  - [x] Add CORS allowlist via `API_ALLOWED_ORIGIN` with credentials enabled.
- [x] Verify local run (AC: 1, 2)
  - [x] Ensure API can boot with local env values and connect to DB.
  - [x] Confirm health and readiness endpoints return expected payloads.
...
| 2026-01-18 | v0.1 | Added API service skeleton with DB and middleware. | James |
...
- `pnpm -C packages/api build` (local run succeeded)
- `node packages/api/dist/server.js` (local run succeeded)

### Completion Notes List
- Added Express API skeleton, health endpoints, and error handling.
- Added Knex DB layer with MariaDB default + PostgreSQL support.
- Added request ID logging and CORS allowlist.
...
- packages/api/src/db/migrations/.gitkeep
