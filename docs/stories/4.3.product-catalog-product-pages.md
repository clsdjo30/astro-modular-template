...
## Tasks / Subtasks
- [x] Add product API routes (AC: 1, 2)
  - [x] Implement `GET /api/v1/products` and `GET /api/v1/products/{id}`.
  - [x] Add admin-only CRUD endpoints (POST/PATCH/DELETE) even without full admin UI.
  - [x] Add Zod validation for product payloads.
- [x] Add frontend service layer (AC: 3)
  - [x] Add `apiClient` and `productService` under `templates/presets/commerce/src/services/`.
  - [x] Use `PUBLIC_API_BASE_URL` and credentials for fetch.
- [x] Add product pages (AC: 2)
  - [x] Product list: `/products` -> `templates/presets/commerce/src/pages/products/index.astro`
  - [x] Product detail: `/products/[id]` -> `templates/presets/commerce/src/pages/products/[id].astro`
  - [x] Render list/detail using the service layer and base layout.
...
| 2026-01-18 | v0.1 | Added product API endpoints and commerce pages. | James |
...
- Product endpoints exercised with and without auth cookie

### Completion Notes List
- Added product CRUD endpoints with admin protection and Zod validation.
- Added commerce preset services and product list/detail pages.

### File List
- packages/api/src/services/products.service.ts
- packages/api/src/controllers/products.controller.ts
- packages/api/src/routes/products.routes.ts
- packages/api/src/app.ts
- templates/presets/commerce/src/services/apiClient.ts
- templates/presets/commerce/src/services/productService.ts
- templates/presets/commerce/src/pages/products/index.astro
- templates/presets/commerce/src/pages/products/[id].astro
