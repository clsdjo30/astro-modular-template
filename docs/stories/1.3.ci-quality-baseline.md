...
## Tasks / Subtasks
- [x] Add CI workflow for generator repo (AC: 1, 2)
  - [x] Create `.github/workflows/ci.yaml` with `pnpm install`, `pnpm lint`, `pnpm typecheck`, `pnpm astro check`, `pnpm build`.
  - [x] Pin pnpm setup to 10.0.0 in CI.
- [x] Ensure base template scripts exist and pass locally (AC: 1, 2, 3)
  - [x] Verify `lint`, `typecheck`, `astro check`, and `build` scripts in base template.
  - [x] Confirm clean run of all commands on a generated project (no manual edits).
- [x] Add lint/format config to base template (AC: 3)
  - [x] ESLint config aligned with ESLint 9.0.0.
  - [x] Prettier config aligned with Prettier 3.3.0.
- [x] Add minimal test command (AC: 2)
  - [x] Ensure `pnpm test` runs cleanly (placeholder ok).
...
- CI not executed locally; relies on workflow run for generated project validation.
- GitHub Actions CI passed after ESLint ignore update for `.astro/`.
- Added `@astrojs/check` to base template to allow `pnpm astro check` in CI.

### Completion Notes List
- Added CI workflow to generate a project via CLI and run lint/typecheck/astro check/build.
- Base template already has lint/format configs and scripts from Story 1.1.
- CI validated generated project; lint issue resolved by ignoring `.astro/`.
- CI requires `@astrojs/check` in base template devDependencies.
